...
//
// Skips the max `outlier_ratio` values.
...
KolmogorovSmirnovTest::Result KolmogorovSmirnovTest::Summary(
  ...
  const auto lower_delta =
      OneSidedDistributionDistance(a_cdf, b_cdf, params_.min_effect);

  // Check if sometimes, b > a + min_effect, or if always, b < a +
  // min_effect.
  const auto higher_delta =
      OneSidedDistributionDistance(b_cdf, a_cdf, params_.min_effect);
